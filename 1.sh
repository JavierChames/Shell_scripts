			...
			#mkdir $MOVEDIR$SeriesName
				...
				Music_torrent_FLAC=`transmission-remote -t $TORRENTID -if | grep flac` #check if its a flac file
				   if  [[ "$Music_torrent_MP3" ]] || [[ "$Music_torrent_FLAC" ]]; then 
					...
					#app=true
					Isamovie=
					if [[ `transmission-remote -t $TORRENTID -i  | grep 1080p` ]] || [[`transmission-remote -t $TORRENTID -i  | grep 1080P`]] || [[ `transmission-remote -t $TORRENTID -i  | grep 720p` ]] || [[ `transmission-remote -t $TORRENTID -i  | grep 720P` ]]; then
#				   if	 [[ $app == false ]]; then		   
